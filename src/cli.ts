...
import { ensureProjectInitialized, getPlansDir, saveConfig } from './config.js';
import { readPlan, writePlan, listPlans, parseFrontmatter, serializePlan } from './plan.js';
import { rebuildAndWriteIndex } from './index-graph.js';
...
import { normalizePlanId, resolvePlanPath } from './plan-path.js';
import { generateInteractiveGraph, generateTerminalGraph, generateMermaid, generateDot } from './graph.js';
...
program
  ...
  .action(() => {
    ...
    if (existsSync(anchorPath)) {
      const plan = readPlan(plansDir, 'anchor');
      const content = serializePlan(plan.frontmatter, plan.body);
      console.log(content);
    }
...
program
  ...
  .action(async (name: string, options) => {
    ...
    name = normalizePlanId(name);
    resolvePlanPath(plansDir, name);
    let content: string;
    ...
    if (options.from) {
      ...
      const editorDir = mkdtempSync(path.join(tmpdir(), 'anchormd-edit-'));
      const tmpFile = path.join(editorDir, 'plan.md');
      ...
      try {
        ...
        throw new Error('Editor exited with an error');
      } finally {
...
program
  ...
  .action(async (query: string, options) => {
    const { projectRoot, config } = ensureProjectInitialized();
    listPlans(getPlansDir(projectRoot));
    if (config.qmd && !config.collectionName) {
      throw new Error('QMD collection is not configured. Run `anchormd reindex` to register this project before searching.');
    }
...
program
  ...
  .action((options) => {
    ...
    if (options.text) {
      ...
      console.log(generateMermaid(graph, plansDir));
    } else if (options.dot) {
      console.log(generateDot(graph, plansDir));
    } else {
      const html = generateInteractiveGraph(graph, plansDir, projectRoot);
